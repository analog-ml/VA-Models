    ...
    with open(filename, "r") as file:
        ...
        for line in lines:
            ...
            if (
                ...
                and not line.__contains__("frequency")
            ):
                parts = line.split()
                freq = float(parts[0])

                voutp_real = float(parts[1])
                voutp_imag = float(parts[2].strip("i"))

                vin_diff = 1.0  # + 1j  # Placeholder for differential input voltage
                # print("vinp:", vinp_real, vinp_imag)
                # print("vinn:", vinn_real, vinn_imag)
                print("vin_diff:", vin_diff)
                vout_diff = complex(
                    voutp_real, voutp_imag
                )  # - complex(voutn_real, voutn_imag)
                # Compute magnitudes
